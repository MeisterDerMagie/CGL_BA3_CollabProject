...
namespace qtools.qhierarchy.pcomponent
{
    public class QComponentsComponent: QBaseComponent
    {
        ...
        public override QLayoutStatus layout(GameObject gameObject, QObjectList objectList, Rect selectionRect, ref Rect curRect, float maxWidth)
        {
            ...
            if (ignoreScripts != null)
            {
                for (int i = 0; i < currentComponents.Length; i++)
                {
                    ...
                    if(currentComponents[i] != null) componentName = currentComponents[i].GetType().FullName;
                    bool ignore = false;
            ...
            float totalWidth = 2 + (rect.width+2) * componentsToDraw;
        ...
        {
            for (int i = components.Count - 1; i >= components.Count - componentsToDraw; i--)
            {
                ...
                if(component == null) Debug.LogWarning($"Missing Script on GameObject \"{gameObject}\"", gameObject);
        ...
        public override void eventHandler(GameObject gameObject, QObjectList objectList, Event currentEvent)
        {
            if (currentEvent.isMouse && currentEvent.button == 0 && eventRect.Contains(currentEvent.mousePosition))
            {
                ...
                {
                    //von mir hinzugefügt, um die Components anders herum anzuzeigen. Um es wieder so zu machen wie im Original: componentsReversed in den nächsten Zeilen durch components ersetzen 
                    var componentsReversed = new List<Component>(components);
                    componentsReversed.Reverse();
                    //
                    //int id = Mathf.FloorToInt((currentEvent.mousePosition.x - eventRect.x) / rect.width) + componentsReversed.Count - 1 - componentsToDraw + 1;
                    int id = Mathf.FloorToInt((currentEvent.mousePosition.x - eventRect.x) / rect.width) + componentsReversed.Count - 1 - componentsToDraw;

                    try
                    {
                        PropertyInfo propertyInfo = componentsReversed[id].GetType().GetProperty("enabled");
                        bool enabled = (bool)propertyInfo.GetGetMethod().Invoke(componentsReversed[id], null);
                        Undo.RecordObject(componentsReversed[id], enabled ? "Disable Component" : "Enable Component");
                        propertyInfo.GetSetMethod().Invoke(componentsReversed[id], new object[] { !enabled });
                    }
                    catch {}
